// KKS AI Smart Trainer — local backend connection layer
// Talks to ComfyUI (http://127.0.0.1:8188) and the BLIP caption server (http://127.0.0.1:8189).
// Falls back to mock mode when the local backend is offline (browser preview without GPU).
...
export async function getBackendStatus(): Promise<{ comfyui: boolean; caption: boolean }> {
  const [comfyui, caption] = await Promise.all([checkComfyUI(), checkCaptionServer()]);
  return { comfyui, caption };
}
...
// ── Fetch generated image as blob URL ─────────────────────────────────────────
